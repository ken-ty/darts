...
class _OutshotListPageState extends State<OutshotListPage> {
  ...
  List<OutShot> generateDoubleOutCombinations() {
    return DoubleOutRoutes.routes.entries
        .map(
          (entry) => OutShot(
            score: entry.key,
            dartsNeeded: entry.value.length,
  ...
  List<OutShot> generateDoubleOutCombinationsForMyOut() {
  ...
  // プロモデル用（最短ダブルアウトのみ）
  List<OutShot> generateDoubleOutCombinationsForPro() {
  ...
  List<OutShot> generateOutshotCombinationsFromTable(
  ...
  List<OutshotSet> get dummySets => [
    OutshotSet(
      id: '1',
      name: 'マイアウトショット',
      labelIds: ['beginner', 'soft'],
      combinations: generateOutshotCombinationsFromTable(outshotTable, 2),
      createdAt: DateTime.now(),
    ),
    OutshotSet(
      id: '2',
      name: 'プロモデル',
      labelIds: ['intermediate', 'double_out'],
      combinations: generateOutshotCombinationsFromTable(outshotTable, 3),
      createdAt: DateTime.now(),
    ),
  ];
